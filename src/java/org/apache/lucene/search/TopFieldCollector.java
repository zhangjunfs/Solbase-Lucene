...
public abstract class TopFieldCollector extends TopDocsCollector {
  ...
  private static class OneComparatorNonScoringCollector extends 
    ...
    final void updateBottom(int doc, int[] sorts) {
      // bottom.score is already set to Float.NaN in add().
      bottom.docID = docBase + doc;
      DocIdSetIterator.copySorts(sorts, bottom.sorts);
  ...
  private static class OutOfOrderOneComparatorNonScoringCollector extends
    ...
    public void collect(int doc, int[] sort) throws IOException {
      ++totalHits;
      if (queueFull) {
  ...
  private static class OneComparatorScoringMaxScoreCollector extends
    ...
    final void updateBottom(int doc, float score, int[] sort) {
      bottom.docID = docBase + doc;
      bottom.score = score;
      bottom = (Entry) pq.updateTop();
  ...
  final void add(int slot, int doc, float score, int[] sorts) {
    bottom = (Entry) pq.add(new Entry(slot, docBase + doc, score, sorts));
  ...
  protected void populateResults(ScoreDoc[] results, int howMany) {
    if (fillFields) {
      ...
      for (int i = howMany - 1; i >= 0; i--) {
        ...
        results[i] = new FieldDoc(entry.docID, entry.score, entry.sorts);
      }
    }
  }
